...
.parseISO8601 <- function(x, start, end, tz="") {
        #error.chain<-paste("\n accessed parseISO8601\n",capture.output(x),"\t");
        #for (counter in (1:(sys.nframe()-1)))
        #        error.chain<-paste(error.chain,counter,capture.output(sys.call(counter)),sep="\t");
		#logerror(error.chain);
 ...
 parse.side <- function(x, startof) {
   ...
   if(!is.null(secs))
   {
     if(0 == length(grep(".",S,ignore=FALSE, fixed = TRUE)))
     { # there are no milliseconds
         subsec=.99999;
     }
     else { # there are milliseconds
         subsec = secs %% 1;
         secs = floor(secs);
     }
   }
